...
export const runner = {
  run(actionName, params) {
    ...
    const listResult = exec("/builtin/document/entity-list-documents", JSON.stringify({}));
    ...
    const summary = documents.slice(0, input.limit || 10).map((doc) => {
      const getResult = exec("/builtin/document/entity-get-document", JSON.stringify({ path: doc.path, name: doc.name }));
