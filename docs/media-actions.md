...
- **MediaDocument de-duplication** — 2026-08-22. `install.solx` had always separately registered its own `/packages/solx-media/MediaDocument` (used for the actions' `result_type_ref`), while `persist.rs` validated/saved against the `/builtin/types/MediaDocument` copy seeded in `solx-types/seed.rs` — two near-identical schemas, only one of them actually load-bearing at runtime. Consolidated onto the package-scoped type: the `/builtin/types/MediaDocument` `SeedType` and its `media_document_schema()` were removed from `solx-types/src/seed.rs` (the built-in seed's `title`/`description` field annotations were folded into `install.solx`'s copy first, so nothing was lost), and `persist.rs` now points at `/packages/solx-media/MediaDocument`. The type now lives and dies with the package, installed/uninstalled via `install.solx`/`uninstall.solx` like everything else solx-media owns.
