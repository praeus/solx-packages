...
const INNER_BUILD_ENV: &str = "SOLX_MEDIA_INNER_BUILD";
const SKIP_AUTOBUILD_ENV: &str = "SOLX_MEDIA_SKIP_AUTOBUILD";

fn main() {
    println!("cargo:rerun-if-changed=src/main.rs");
    println!("cargo:rerun-if-changed=Cargo.toml");
    println!("cargo:rerun-if-changed=build.rs");
    println!("cargo:rerun-if-changed=src/sha256_inline.rs");
    ...
    if env::var_os(SKIP_AUTOBUILD_ENV).is_some() {
        println!(
            "cargo:warning=Skipping solx-media auto-build because {} is set",
            SKIP_AUTOBUILD_ENV
        );
        return;
    }

    // Mirror solx-omniparse: only stage on --release so dev/check is fast.
    if env::var("PROFILE").as_deref() != Ok("release") {
        return;
    }

    if let Err(err) = build_and_stage() {
        println!("cargo:warning=solx-media staging failed: {err}");
    }
}
...
fn build_and_stage() -> Result<(), String> {
    ...
    // Cargo's default binary name = bin name (`solx-media`).
    let bin_name = if cfg!(windows) { "solx-media.exe" } else { "solx-media" };
    ...
    println!(
        "cargo:warning=staged solx-media binary at {}",
        staged.display()
    );
