...
/// steps phase's user message. Independent of
...
/// Everything [`run_search_with`] needs, in one value.
///
/// Deliberately not derived from the request params: one `prompt` call runs
/// several look-ups with different scopes and different reach (an action
/// look-up and a document look-up want different path prefixes, and a type
/// filter is meaningless for the former), so the spec is built per look-up by
/// the search phase rather than once per request.
#[derive(Debug, Clone)]
...
pub struct Hit {
    ...
    /// so this package can tell its own document kinds (skills, memories,
    /// sessions) apart from evidence, and adding a field to the documented
    /// result shape for that would be a change nothing asked for.
    pub type_ref: Option<String>,
...
/// The one place a hit list becomes prompt text.
...
/// type, and the search phase calls [`run_search_with`] once per look-up, so
/// an action that two look-ups both surface (a common helper like
/// `search-documents` is a likely one) would otherwise pay for its schema
/// twice over. `None` is cached too, so a reference that is absent or fails to
...
/// it.
///
/// Scoped by the caller, not global: the search phase builds exactly one and
/// threads it through every look-up in the turn, which is where the sharing
/// actually pays off.
...
/// `details` missing rather than failing the whole look-up over what is
...
/// whole look-up over what is strictly additional context.
fn fetch_type_schema(host: &dyn Host, type_ref: &str) -> Option<Value> {
    ...
    let call = match host.exec(TYPE_GET_REF, &payload) {
        ...
        Ok(c) => {
            host.log(&format!(
                "solx-prompt: entity-get-type {type_ref} failed: {}",
                c.message.unwrap_or_default()
            ));
            return None;
        }
        Err(e) => {
            host.log(&format!("solx-prompt: entity-get-type {type_ref} failed: {e}"));
            return None;
        }
    };
...
fn search_documents(host: &dyn Host, spec: &SearchSpec, term: &str) -> Result<Vec<Hit>, Outcome> {
    ...
    host.log(&format!("solx-prompt: search-documents q={term:?}"));
    let call = host
...
fn search_actions(host: &dyn Host, spec: &SearchSpec, term: &str) -> Result<Vec<Hit>, Outcome> {
    ...
    host.log(&format!("solx-prompt: search-actions q={term:?}"));
    let call = host
