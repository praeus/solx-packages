//! Search-term helpers, all local — this module makes no llm call.
//!
//! The intent phase's `format` schema marks each look-up's `terms` as
//! `required`, so a compliant model supplies them and there is no term-
//! generation round trip to pay for. What is left here is the handling for a
//! model that ignored the schema anyway: [`parse_terms`] reads a term list out
//! of loosely-shaped content, and [`fallback_terms`] derives one from the
//! question itself when there was nothing to read.
//!
//! Deriving them locally rather than asking the model again is the whole point.
//! A third llm call would be a 50% increase in this pipeline's call budget to
//! recover from a case that a schema already covers.
...
///
/// The intent schema marks each look-up's `terms` as `required`, so a
/// compliant model always supplies them. This covers the model that ignores
/// `format` anyway — and it covers it locally, because the alternative is a
/// third llm call to recover from a case the schema already handles.
///
/// Single words only, for the reason the intent prompt spells out:
...
/// phrase where the prompt asked for keywords can therefore drive a look-up
...
/// matched nothing at all and produced no plan from a look-up that had a
...
///    rather than the words that decide whether the look-up finds anything at
...
/// single word could be added, and the look-up searched only the phrases that
/// match nothing.
///
/// Applied here rather than only asked for in the prompt, because the prompt
/// already asks and a model that ignored it still has to search something.
/// Safe to rewrite silently in this pipeline: terms are an implementation
/// detail of the search phase and are not part of the documented result, so
/// nothing downstream reports them back as what the model said.
...
pub fn parse_terms(content: &str, max_terms: usize) -> Option<Vec<String>> {
...
mod tests {
    ...
    fn a_full_budget_of_phrases_still_gets_searched_by_word() {
        ...
        // one of those phrases matches nothing, so the look-up found nothing
        // at all.
