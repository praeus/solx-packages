...
//!   drives the `Some` endpoints through `/builtin/web/stream/*`
...
pub enum StreamKind {
    ...
    Chat,
    /// `/api/pull` — each chunk is a standalone status object
    /// (`{"status": "...", "completed": N, "total": M}`). There is nothing
    /// to concatenate; the last chunk is the result.
    Progress,
}
...
pub fn build_body(ep: &Endpoint, params: &Value) -> Result<Option<Value>, Vec<String>> {
    ...
    // Endpoints with no streaming mode at all (`list_models`) never got a
    // `stream` field before and still don't — sending it would just be
    // noise. Streaming endpoints always get `true` now, unconditionally,
    // mirroring the old unconditional `false`.
