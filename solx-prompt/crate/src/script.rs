...
//! this module checks that list against what the search phase actually
//! surfaced. That is the whole job: a caller executes the returned
...
//!   among the actions the search phase surfaced, and says so in the notes.
...
/// What the search phase actually surfaced — the ground truth a proposed step
/// is checked against.
///
/// Bundled rather than passed as two parallel slices because they are two
/// views of one thing (the actions this turn may legitimately call) and are
/// always built together, by the search phase.
...
/// Validate one model-proposed entry against what the search phase surfaced.
///
...
/// by [`assemble`]'s caller from the notes it returns.
pub fn assemble(
    title: Option<String>,
