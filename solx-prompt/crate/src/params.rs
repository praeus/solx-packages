//! Shared param plumbing: the llm connection carrier, and the search-scope enum.
//!
//! [`Params`] is deliberately *only* a connection carrier. Everything about
//! what to search, how much of it to return and which prompts to use lives on
//! `PromptParams`, which builds one of these purely so [`apply_llm_overrides`]
//! and [`crate::llm::call`] work against a small, stable surface. `llm.rs`
//! reads exactly one field off it (`llm_action_ref`) and hands the rest to
//! `apply_llm_overrides`, which is why keeping this struct narrow costs
//! nothing.
...
/// What a look-up searches. Both phases share it: the intent phase labels each
/// look-up it proposes, and the search phase turns that label into a
/// [`crate::search::SearchSpec`].
...
/// The llm connection, and nothing else.
#[derive(Debug, Clone)]
pub struct Params {
    /// Action reference for the chat-completion action both phases drive.
    /// Defaults to `solx-ollama`'s `ollama-chat`, but any action taking the
    /// same `{model, messages, format?}` shape and returning Ollama-style
...
impl Default for Params {
    fn default() -> Self {
        Params {
            llm_action_ref: DEFAULT_LLM_ACTION_REF.to_string(),
            base_url: None,
            api_key: None,
...
mod tests {
    ...
    #[test]
    fn every_connection_override_reaches_the_payload() {
        let params = Params {
            llm_action_ref: "/packages/other/chat".to_string(),
            base_url: Some("http://localhost:11434".to_string()),
            api_key: Some("k".to_string()),
            auth_secret_name: Some("s".to_string()),
            headers: Some(json!({ "X-Trace": "1" })),
            timeout_secs: Some(60),
            options: None,
        };
        let merged = apply_llm_overrides(json!({ "model": "m" }), &params);
        assert_eq!(merged["base_url"], json!("http://localhost:11434"));
        assert_eq!(merged["api_key"], json!("k"));
        assert_eq!(merged["auth_secret_name"], json!("s"));
        assert_eq!(merged["headers"], json!({ "X-Trace": "1" }));
        assert_eq!(merged["timeout_secs"], json!(60));
        // The ref is how the call is *addressed*, never part of its payload.
        assert!(merged.get("llm_action_ref").is_none());
    }
}
